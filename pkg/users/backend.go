...
func (ub *UserBackend) GetUserForSearchEngine(id int) (*SearchUser, error) {
	query := `SELECT 
       ...
       (SELECT SUM(EXTRACT(EPOCH FROM (left_time - join_time))) FROM user_room_logs WHERE user_id = id AND join_time <= NOW() - INTERVAL '7 DAYS' AND visibility = 'public') FROM users WHERE id = $1;`
